...
-   [4.4 バックグラウンドのあるポアソン分布](#バックグラウンドのあるポアソン分布)
-   [4.5 カウンタの感度](#カウンタの感度)
-   [4.6 ポアソン分布の信頼区間とその問題点](#ポアソン分布の信頼区間とその問題点)
-   [4.7 Feldman-Cousinsの信頼区間](#feldman-cousinsの信頼区間)
...
ポアソン分布の平均はλ、標準偏差は√λと述べた。では、平均λ = 0のときは、標準偏差も0になるのだろうか？

2項検定のbinom.test関数があるように、ポアソン分布を使った検定、ポアソン検定のpoisson.test関数がある。binom.test関数で信頼区間が表示されたように、poisson.test関数でも信頼区間が表示される。λ = 10のポアソン分布で試してみよう。
...
95%信頼区間は、4.8～18.4であることがわかる。
さて、平均λ = 0となるときも、信頼区間はあるのだろうか？もし、標準偏差が√λに従うなら、標準偏差も0になる、つまりばらつきが全くない確率分布になりそうだが…ポアソン検定で確かめてみよう。
...
```
    ...
    ## 
    ## data:  0 time base: 1
    ## number of events = 0, time base = 1, p-value = 1
    ## alternative hypothesis: true event rate is not equal to 1
    ## 95 percent confidence interval:
    ##  0.000000 3.688879
    ## sample estimates:
...
信頼区間は0～3.7ほどになった。つまり、ある一定期間で起こったある事象が0回だったとしても、今後同じ期間で3回ほど起きる可能性は否定できない。例えば、これまでの人生(例えば、30年だとしよう)で、一回も鳩のフンが頭に落ちなかったとしても、今後の30年間、つまり60歳になるまでに3回フンが頭に落ちてくることも否定はできない(もちろん0回のこともある)。

4.4 バックグラウンドのあるポアソン分布
--------------------------------------
